...
class Factory
{
    ...
    public static function parent(Process $process, LoopInterface $loop, $interval = self::INTERVAL, $timeout = self::TIMEOUT)
    {
        $process->start($loop, $interval);

        return \WyriHaximus\React\tickingPromise($loop, $interval, [$process, 'isRunning'])->then(function () use ($process) {
    ...
    public static function child(LoopInterface $loop)
    {
